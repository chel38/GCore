...
Версия `0.3.0-alpha` сохраняет backward-compatible Identity API v1 и повышает
network protocol модуля до v2. Причина: отправка email больше не завершает
регистрацию — обязателен одноразовый код, созданный сервером.
...
```text
registration_required -> email -> registering
...
До проверки кода активный account не создаётся. TTL — 10 минут, максимум пять
...
protocol-v2 ingress events `verifyEmail` и `resendVerification` имеют strict
schema validation и отдельные rate limits.
