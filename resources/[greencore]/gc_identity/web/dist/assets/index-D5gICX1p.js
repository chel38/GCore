(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),t.credentials=e.crossOrigin===`use-credentials`?`include`:e.crossOrigin===`anonymous`?`omit`:`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e={"GC-IDENTITY-REGISTRATION-INVALID":`Проверьте адрес электронной почты.`,"GC-IDENTITY-EMAIL-TAKEN":`Этот адрес уже используется.`,"GC-IDENTITY-CHARACTER-INVALID":`Имя или фамилия имеют недопустимый формат.`,"GC-IDENTITY-CHARACTER-LIMIT":`Достигнут лимит персонажей.`,"GC-IDENTITY-CHARACTER-NOT-OWNED":`Персонаж не принадлежит вашему аккаунту.`,"GC-IDENTITY-RATE-LIMIT":`Слишком много запросов. Подождите немного.`,"GC-IDENTITY-DATABASE-UNAVAILABLE":`Сервис профилей временно недоступен.`,"GC-IDENTITY-CLIENT-REQUEST-PENDING":`Предыдущий запрос ещё выполняется.`,"GC-IDENTITY-NUI-TRANSPORT":`Не удалось связаться с игровым клиентом.`};function t(e){return e.replace(/[&<>'"]/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,"'":`&#39;`,'"':`&quot;`})[e]??e)}function n(e){return`${e.firstName} ${e.lastName}`}function r(r,i){let a=null,o=null,s=null,c=!1,l=()=>`
    ...
    </section>`,u=()=>`<div class="alert" role="alert"><span>${t(e[s??``]??`Запрос отклонён. Повторите попытку.`)}</span><button data-action="dismiss-error" aria-label="Закрыть">×</button></div>`,d=()=>c?`
    ...
    <section class="identity-shell" data-view="registration">
      ...
      ${d()}
    </section>`,p=()=>{let e=a?.characters??[],r=a?.limits.maxCharacters??0,i=e.length<r,c=e.map(e=>`
      ...
      <section class="identity-shell" data-view="characters">
        ...
        ${d()}
      </section>`},m=()=>{r.innerHTML=!a||[`uninitialized`,`loading`,`authorized`,`registering`,`character_selected`].includes(a.state)?l():a.state===`registration_required`?f():a.state===`character_required`?p():a.state===`error`?`<section class="identity-shell"><div class="panel panel--small text-center"><p class="eyebrow">Ошибка</p><h1>Профиль недоступен</h1>${u()}<button class="button button--secondary" data-action="refresh">Повторить</button></div></section>`:``},h=async(e,t)=>{if(o){s=`GC-IDENTITY-CLIENT-REQUEST-PENDING`,m();return}o=e,s=null,m();try{let n=await i.invoke(e.split(`:`)[0]??e,t);n.ok||(o=null,s=n.code??`GC-IDENTITY-REQUEST-REJECTED`,m())}catch{o=null,s=`GC-IDENTITY-NUI-TRANSPORT`,m()}},g=e=>{let t=e.target;if(!(t instanceof HTMLFormElement))return;e.preventDefault();let n=new FormData(t);t.dataset.form===`registration`?h(`registerAccount`,{email:String(n.get(`email`)??``).trim()}):t.dataset.form===`character`&&h(`createCharacter`,{firstName:String(n.get(`firstName`)??``).trim(),lastName:String(n.get(`lastName`)??``).trim()})},_=e=>{let t=e.target instanceof Element?e.target.closest(`[data-action]`):null;if(!t)return;let n=t.dataset.action;if(n===`select-character`){let e=Number(t.dataset.characterId);h(`selectCharacter:${e}`,{characterId:e})}else n===`ask-exit`?(c=!0,m()):n===`cancel-exit`?(c=!1,m()):n===`confirm-exit`?h(`exit`,{}):n===`dismiss-error`?(s=null,m()):n===`refresh`&&h(`refresh`,{})},v=e=>{e.key===`Escape`&&a?.state!==`ready`&&(c=!c,m())};return r.addEventListener(`submit`,g),r.addEventListener(`click`,_),document.addEventListener(`keydown`,v),m(),{receive(e){e.type===`snapshot`?(a=e.payload,o=null,s=null,c=!1):e.type===`rejected`&&(o=null,s=e.payload.code),m()},destroy(){r.removeEventListener(`submit`,g),r.removeEventListener(`click`,_),document.removeEventListener(`keydown`,v)}}}function i(){return window.GetParentResourceName?.()??`gc_identity`}var a={async invoke(e,t){let n=await fetch(`https://${i()}/${e}`,{method:`POST`,headers:{"Content-Type":`application/json; charset=UTF-8`},body:JSON.stringify(t)});return n.ok?await n.json():{ok:!1,code:`GC-IDENTITY-NUI-TRANSPORT`}}},o=document.querySelector(`#app`);if(!o)throw Error(`gc_identity NUI root is missing`);var s=r(o,a);window.addEventListener(`message`,e=>{(e.data?.type===`snapshot`||e.data?.type===`rejected`)&&s.receive(e.data)}),a.invoke(`ready`,{});